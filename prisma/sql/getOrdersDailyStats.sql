...
WITH
    ...
    -- ✅ Filter orders (PARENTS + SUBORDERS)
    filtered_orders AS (
        SELECT o.*
        FROM "Order" o
        WHERE o.status = 'PAID'
          ...
          AND (
            ...
            OR (
              ...
              OR ($5::time >  $6::time AND ((timezone('Europe/Rome', o.created_at))::time >= $5::time
                                            OR (timezone('Europe/Rome', o.created_at))::time <= $6::time))
            )
          )
    ...
    -- ✅ Product lines: ONE ROW PER ORDER (NO collapsing)
    product_lines AS (
        SELECT
            o.id AS order_id,
    ...
    -- ✅ Daily aggregation (LORDO-correct)
    raw_daily AS (
        SELECT
