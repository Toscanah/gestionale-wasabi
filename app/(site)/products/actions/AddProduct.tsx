...
export default function AddProduct({ onAdd }: { onAdd: (newProduct: ProductWithInfo) => void }) {
  function onSubmit(values: FormValues) {
    fetchRequest<ProductWithInfo>("POST", "/api/products/", "createNewProduct", values).then(
      (product) => {
        if (product) {
          onAdd(product);
