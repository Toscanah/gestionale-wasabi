...
export const CreatePromotionFormSchema = z
  ...
  .refine(
    (data) =>
      data.type !== PromotionType.FIXED_DISCOUNT || (data.fixed_amount && data.fixed_amount > 0),
    {
      message: "L'importo fisso deve essere maggiore di zero",
      path: ["fixed_amount"],
    }
  )
  .refine(
    (data) =>
      ...
      (data.percentage_value && data.percentage_value > 0),
    {
      message: "La percentuale deve essere maggiore di zero",
      path: ["percentage_value"],
    }
  )
  .refine(
    (data) => data.type !== PromotionType.GIFT_CARD || (data.fixed_amount && data.fixed_amount > 0),
    {
